...
public class ConsultarMateria extends JPanel{
    ...
    JTextField textParametroMateria = new JTextField();

    public ConsultarMateria(Connection conexion){
    ...
    public void componentes(Connection conexion){
        ...
        JLabel etiquetaTitulo = new JLabel("Consultar Materias");
        ...
        comboTipoConsulta.setModel(new DefaultComboBoxModel<>(new String[] { "SELECCIONA","ID MATERIA","NOMBRE",
                "AREA"}));
        config.gridx = 2;
        config.gridwidth = 1;
        config.gridy = 1;
        ...
        JTable tablaAlumnos = new JTable(MateriasDAO.modelo);
        config.gridx = 0;
        config.gridwidth = 7;
        config.gridheight = 2;
        config.gridy = 2;
        ...
        config.gridwidth = 1;
        config.gridy = 1;
        this.add(textParametroMateria, config);
        ...
        textParametroMateria.setVisible(false);
    ...
    private void mostrarText(){
        ...
        if(!parametroSeleccionado.equals("SELECCIONA")){
            ...
            textParametroMateria.setVisible(true);
        }
    }
    private void consultarMaterias(Connection conexion) throws SQLException {
        String tipoConsulta = (String) comboTipoConsulta.getSelectedItem();
        String parametro = textParametroMateria.getText();
